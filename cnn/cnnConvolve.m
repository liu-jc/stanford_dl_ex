...
%fprintf('filterDim = %d,imageDim = %d\n',filterDim,imageDim);
% Instructions:
...
for imageNum = 1:numImages
  for filterNum = 1:numFilters
    ...
    %convolvedImage
    %convolvedImage = convolvedImage(filterDim:imageDim,filterDim:imageDim);
    convolvedImage = conv2(im,filter,'valid');
    %%% YOUR CODE HERE %%%
    
    % Add the bias unit
    % Then, apply the sigmoid function to get the hidden activation
    b_mat = repmat(b(filterNum),convDim,convDim);
    convolvedImage = convolvedImage + b_mat;
